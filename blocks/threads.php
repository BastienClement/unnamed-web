...
$result = $db->query("SELECT t.id, t.subject, t.last_post, t.last_poster, p.poster_id FROM {$db->prefix}topics AS t INNER JOIN {$db->prefix}posts AS p ON p.id=t.last_post_id INNER JOIN {$db->prefix}users AS u ON u.id=p.poster_id LEFT JOIN {$db->prefix}forum_perms AS fp ON (fp.forum_id=t.forum_id AND fp.group_id={$pun_user['g_id']}) WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND t.moved_to IS NULL AND t.forum_id NOT IN(16, 17) ORDER BY t.last_post DESC LIMIT 5") or error('Unable to fetch topic info', __FILE__, __LINE__, $db->error());
